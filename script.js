...
for (const miResultado of fechaMiResultado) {
    const divPronostico = document.createElement("div");
    divPronostico.innerHTML = `<label id="partido">PARTIDO 1</label>
                        <ul>
                            <li>
                                <ul>
                        ...
                        <ul>
                            ...
                            <!--button type="submit" name="guardar">Guardar Pronostico</button-->
                        </ul>`;
    document.body.appendChild(divPronostico);
    divPronostico.classList.add("item");
    
    
    /* Agregue un boton al formulario, escucha el evento 'click' y ejecuta 
    la funcion de respuesta mostrando un resultado por consola, 
    pero se ejecuta tres veces con solo click. 
    
    Sospecho que el problema esta en que cada elemento divPronostico tiene 
    que tener un id asociado para que asi se ejecute de manera independiente en cada click.
    */
    
    let boton = document.getElementById("guardarPronostico") 
    boton.addEventListener('click', cargarpronostico);

    function cargarpronostico(){
        console.log("Se ha recibido un nuevo pronostico");
    };
}     
